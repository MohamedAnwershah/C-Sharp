...
class Program
{
    static void Main()
    {
        ...
        List<string> versions = raw_versions.Split(new[] { "\n" }, StringSplitOptions.RemoveEmptyEntries).ToList();
       
        foreach (string curr_ver in versions)
        {
            string raw_segments = GetFiles($"/Users/mohamedanwershah/Downloads/.zenbridge/std/X12/{curr_ver}/Segments");
            List<string> segments = raw_segments.Split(new[] { "\n" }, StringSplitOptions.RemoveEmptyEntries).ToList(); 
            ...
            foreach (string s in segments) 
            {
                ...
                if (!hashDictionary.ContainsKey(cleanHash))
                {
                    hashDictionary[cleanHash] = new List<string>();
                }
                
                hashDictionary[cleanHash].Add(curr_ver);
    ...
    static string GetSha(string filename, string version)
    {
        var psi = new ProcessStartInfo
        {
            ...
            Arguments = $"-c \"dhall hash --file /Users/mohamedanwershah/Downloads/.zenbridge/std/X12/{version}/Segments/{filename}\"",
            UseShellExecute = false,
            CreateNoWindow = false,
            RedirectStandardOutput = true,

            WorkingDirectory = $"/Users/mohamedanwershah/Downloads/.zenbridge/std/X12/{version}/Segments"
        };
