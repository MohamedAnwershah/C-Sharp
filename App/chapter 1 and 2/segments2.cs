...
class Program
{
    static void Main()
    {
        ...
        foreach (var key in hashDictionary.Keys)
        {
            uniqueCount += hashDictionary[key].Count;
            foreach (var innerKey in hashDictionary[key].Keys)
            {
                totalCount += hashDictionary[key][innerKey].Count;
            }
        }
